...
proc vTcl:add_new_custom_stmts {} {
    ...
    foreach c [lsort -unique $vTcl(must_add_custom)] {
        append str "$c = Frame     # To be updated by user with name of custom widget.\n"
    }
    return $str
}
...
"
    if {$vTcl(custom_present)} {
        ...
        append source \
"
$l = Frame     # To be updated by user with name of custom widget.
"
       }
     }
    append source \
...
$vTcl(tab)import $project_name
$vTcl(tab)${project_name}.vp_start_gui()
"
    # Now write it out.
    $vTcl(supp_source_window)  delete 1.0 end
    $vTcl(supp_source_window)  insert end $source
