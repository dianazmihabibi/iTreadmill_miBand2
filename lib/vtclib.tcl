...
proc vTcl:compile_alias {top} {
    global vTcl widget
    set tree [vTcl:list_widget_tree .$top]
...
proc vTcl:DefineAlias {target alias widgetProc top_or_alias cmdalias} {
    ...
    # cause a duplicate alias and so I added the follow to test for
    ...
    #      && [string first "bgerror" $target] == -1
    #      && [string first ".scr" $target] == -1
    #      && [string first ".tSc" $target] == -1)
    #     || [string first ".scr" $last_chunk] > -1} { }
    if {([string first ".bor" $target] == -1)} {
