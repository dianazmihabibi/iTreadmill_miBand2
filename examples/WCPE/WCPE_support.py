#!/usr/bin/env python
# -*- python -*-
# Time-stamp: <2018-08-17 21:50:15 rozen>
...
# is necessary to provide the current time in Raleigh, North
# Carolina where WCPE is located.

# The previous version of the program required the user to install the current
# version of pytz.  That allowed me to use the time facilities in Python to
# find the differences between the time at the station (in Raleigh, NC).
# I have since changed the program to look up the current time in Raleigh
# on the web.  This is a little slower by about half a second.
...
import datetime
import time
#from pytz import timezone   9/13
#import pytz                 9/13
import unicodedata
import os
import copy
...
def display_data(obj):
    ...
    raleigh_time = get_raleigh_date()
    time_delta = get_time_delta(raleigh_time)
    raleigh_date_str = raleigh_time.strftime("%A, %B %d, %Y")
    w_day = raleigh_time.weekday()
    d = dayofweek[w_day]
...
# Regular expressions for analyzing web pages.
time_patt = r'([0-2][0-9]):([0-9][0-9])'
time_re = re.compile(time_patt)

raleigh_time_patt = r'([0-2]*[0-9]):([0-9][0-9]):[0-9][0-9] ([A-Z][A-Z]), ([A-Za-z, 0-9]*)'
raleigh_time_re = re.compile(raleigh_time_patt)

date_patt = r'[A-Za-z ]*([0-9 ][0-9]), ([A-Za-z]*) ([0-9]*)'
...
def get_playlist():
    ...
    comp_time = copy.copy(raleigh_time)
    cnt = -1
    in_guide = 0
    listing_count = 0
    playlist_cnt = 0
...
def convert_starttime_to_localtime(time):
    t = time.split(':')
    copy_eastern = copy.copy(raleigh_time)
    eastern_replace = copy_eastern.replace(hour = int(t[0]), minute = int(t[1]))
    corrected_time = eastern_replace - time_delta
    t_str = corrected_time.strftime("%H:%M")
    return t_str
...
def get_raleigh_date():
    url = 'http://24timezones.com/world_directory/current_raleigh_time.php'
    req = urllib2.Request(url, headers={'User-Agent' : "Magic Browser"})
    con = urllib2.urlopen( req )
    html = con.readlines()
    found = False
    for l in html:
        if l.find('id="currentTime"') > -1:
            match_obj = raleigh_time_re.search(l)
            if match_obj == None:
                print "ERROR 1"
            hour = int(match_obj.group(1))
            minute = int(match_obj.group(2))
            am_pm = match_obj.group(3)
            if am_pm == 'PM' and hour < 12:
                hour += 12
            if am_pm == 'AM' and hour == 12:
                hour -= 12
            date =  match_obj.group(4)

            match_obj = date_re.search(l)
            if match_obj == None:
                print 'ERROR 2'
            ...
            raleigh_time = datetime.datetime(year, month, day,
                                                     hour, minute)
            print 'get_raleigh_date: raleigh_time  =', raleigh_time    # rozen pyp
            return raleigh_time

def get_time_delta(time):
    '''In order to remove the requirement of an up to date pytz, I look
    ...
    to the text box.
    '''
    ea_out = time.strftime("%A,  %H:%M %Z")
    local_now = datetime.datetime.now()
    diff = time - local_now
    diff_seconds = diff.seconds
    diff_hours = diff_seconds / 3600.0
    q_diff = quarter_round(diff_hours)
    time_delta = datetime.timedelta(hours=q_diff)
    return time_delta

def quarter_round(number):
    general = (round(number * 4) / 4.0)
    return general
...
##     print 'convert_time_to_utc: raleigh_date_str =', raleigh_date_str    # rozen   pyp
