...
class MiBand2(Peripheral):
    ...
    def get_steps(self):
        ...
        a = char.read()
        steps = struct.unpack('h', a[1:3])[0] if len(a) >= 4 else None
        #meters = struct.unpack('h', a[5:7])[0] if len(a) >= 8 else None
        #fat_gramms = struct.unpack('h', a[2:4])[0] if len(a) >= 5 else None
        # why only 1 byte??
        #callories = struct.unpack('b', a[9])[0] if len(a) >= 9 else None
        return steps
#        return {
#            "steps": steps,
#            "meters": meters,
